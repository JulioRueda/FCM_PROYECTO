...
def run_experiment(cfg: DictConfig):
    ...
    # select Tbaseline and Tpartitions ------------------------------------------------------------------
    ...
    # print(type(T['y']))
    # print(idxs)
    N_T  = int(0.1*idxs.shape[0])
    choices = np.arange(idxs.shape[0])
    idxsT_base = np.random.choice(choices, size=N_T, replace=False, p=None)
    idxsT_part  = np.setdiff1d(choices, idxsT_base) #complements  
    T_base        = {'X':T['X'][idxsT_base,:], 'y':T['y'][idxsT_base], 'idxs':T['idxs'][idxsT_base,:]}
    T             = {'X':T['X'][idxsT_part,:], 'y':T['y'][idxsT_part], 'idxs':T['idxs'][idxsT_part,:]}
    # select Tbaseline and Tpartitions ------------------------------------------------------------------

    M, elapsed = AdjustNeuralNetworkWeights(M, T_base, V)
    f1_macro_T   = f1_score(y_true=T_base['y'], y_pred=M.predict_class(T_base['X']), average='macro')
